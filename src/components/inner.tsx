...
export function DarkHero({
  ...
  const content = (
    <>
      ...
      <h1 style={{ ...rise(".85s", "entry 0% cover 24%"), fontFamily: serif, fontWeight: 400, fontSize: split ? 46 : 52, lineHeight: 1.06, letterSpacing: "-0.02em", color: "#FFFFFF", margin: "0 0 22px" }}>
        {title.before}
        {title.italic && <span style={{ fontStyle: "italic", color: "#16B9A6" }}>{title.before ? " " : ""}{title.italic}</span>}
        {title.after}
      </h1>
      <p style={{ ...rise(".85s", "entry 0% cover 28%"), fontSize: 18, lineHeight: 1.7, color: "rgba(226,232,241,0.72)", maxWidth: 700, margin: split ? "0 0 32px" : "0 auto 32px" }}>{subtitle}</p>
  ...
  return (
    <section style={{ position: "relative", overflow: "hidden", background: "radial-gradient(120% 100% at 50% 0%,#13294a 0%,#0B1A2E 52%,#081320 100%)" }}>
      ...
      {split ? (
        <div className="nm-grid-2" style={{ position: "relative", maxWidth: 1180, margin: "0 auto", padding: "80px 32px 84px", display: "grid", gridTemplateColumns: "1.02fr 0.98fr", gap: 56, alignItems: "center", textAlign: "left" }}>
          <div>{content}</div>
          <div>{children}</div>
        </div>
      ) : (
        <>
          <div style={{ position: "relative", maxWidth: 1100, margin: "0 auto", padding: children ? "84px 32px 36px" : "88px 32px 70px", textAlign: align }}>{content}</div>
          {children}
        </>
      )}
    </section>
...
export function CtaBand({ heading, subtext, primary, secondary }: { heading: string; subtext?: string; primary: Btn; secondary?: Btn }) {
  return (
    <section style={{ background: "linear-gradient(165deg,#0A1424,#0B1A2E)", padding: "92px 32px", borderTop: "1px solid rgba(226,232,241,0.08)" }}>
      <div style={{ maxWidth: 780, margin: "0 auto", textAlign: "center" }}>
        <h2 style={{ ...rise(".85s", "entry 0% cover 30%"), fontFamily: serif, fontWeight: 400, fontSize: 38, lineHeight: 1.12, color: "#FFFFFF", margin: subtext ? "0 0 14px" : "0 0 28px" }}>{heading}</h2>
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <div className="nm-overview-grid" style={{ display: "grid", gridTemplateColumns: "0.36fr 0.64fr", gap: 56 }}>
            <div style={{ ...rise(".8s", "entry 0% cover 24%") }}>
              ...
              <div style={{ fontFamily: serif, fontSize: 27, lineHeight: 1.2, color: "#0A1424", letterSpacing: "-0.01em" }}>{data.overview.heading}</div>
            </div>
            <div>
              {data.overview.paras.map((p, i) => (
                <p key={i} style={{ ...rise(".8s", `entry 0% cover ${28 + i * 4}%`), fontSize: 17, lineHeight: 1.78, color: "#475569", margin: "0 0 22px" }}>{p}</p>
              ))}
              <div style={{ ...rise(".8s", "entry 0% cover 36%"), borderLeft: "3px solid #16B9A6", padding: "6px 0 6px 24px", marginTop: 6 }}>
                <div style={{ fontFamily: serif, fontStyle: "italic", fontSize: 25, lineHeight: 1.32, color: "#0A1424" }}>{data.overview.pullquote}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: "#FFFFFF", padding: "100px 32px", borderTop: "1px solid #E2E8F1" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          ...
          <div className="nm-feature-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 20 }}>
            {data.features.map((c) => (
              <div key={c.name} className="nm-feature-card" style={{ ...rise(".8s", "entry 0% cover 30%"), background: "#FFFFFF", border: "1px solid #E2E8F1", borderRadius: 15, padding: 30 }}>
                <h3 style={{ fontFamily: sans, fontWeight: 600, fontSize: 18, color: "#0A1424", margin: "0 0 10px", letterSpacing: "-0.01em" }}>{c.name}</h3>
                <p style={{ fontSize: 15, lineHeight: 1.66, color: "#64748b", margin: 0 }}>{c.desc}</p>
              </div>
            ))}
          </div>
          {data.stack && data.stack.length > 0 && (
            <div style={{ ...rise(".8s", "entry 0% cover 38%"), marginTop: 30, background: "#FFFFFF", border: "1px solid #E2E8F1", borderRadius: 14, padding: "24px 28px" }}>
