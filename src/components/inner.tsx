...
export function DarkHero({
  ...
  const content = (
    <>
      {badge && (
        <div style={{ ...rise(".65s", "entry 0% cover 16%"), display: "inline-block", fontFamily: mono, fontSize: 10.5, letterSpacing: "0.18em", color: "#FFFFFF", background: "linear-gradient(180deg,#9B5DE5,#6D28D9)", borderRadius: 9999, padding: "6px 14px", marginBottom: 18, fontWeight: 600 }}>{badge}</div>
      )}
      <div style={{ ...rise(".7s", "entry 0% cover 18%"), fontFamily: mono, fontSize: 11.5, letterSpacing: "0.24em", color: "#9B5DE5", marginBottom: 24 }}>{eyebrow}</div>
      <h1 style={{ ...rise(".85s", "entry 0% cover 24%"), fontFamily: serif, fontWeight: 600, fontSize: split ? "clamp(30px, 6.2vw, 46px)" : "clamp(30px, 6.4vw, 52px)", lineHeight: 1.06, letterSpacing: "-0.02em", color: "#FFFFFF", margin: "0 0 22px" }}>
        {title.before}
        {title.italic && <span style={{ fontStyle: "italic", color: "#9B5DE5" }}>{title.before ? " " : ""}{title.italic}</span>}
        {title.after}
      </h1>
      ...
      <div style={{ ...rise(".85s", "entry 0% cover 32%"), display: "flex", gap: 13, justifyContent: split ? "flex-start" : "center", flexWrap: "wrap" }}>
        <Link href={primary.href} className="nm-cta-primary" style={{ padding: "14px 26px", background: "linear-gradient(180deg,#9B5DE5 0%,#6D28D9 100%)", color: "#FFFFFF", fontFamily: sans, fontSize: 14.5, fontWeight: 600, textDecoration: "none", borderRadius: 9999, boxShadow: "0 10px 30px rgba(138,57,192,0.35)" }}>{primary.label}</Link>
        {secondary && (
...
export function CtaBand({ heading, subtext, primary, secondary }: { heading: string; subtext?: string; primary: Btn; secondary?: Btn }) {
  return (
    <section style={{ background: "linear-gradient(165deg,#0B0420,#120428)", padding: "clamp(52px, 11vw, 92px) clamp(20px, 5vw, 32px)", borderTop: "1px solid rgba(226,232,241,0.08)" }}>
      <div style={{ maxWidth: 780, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ ...rise(".85s", "entry 0% cover 34%"), display: "flex", gap: 13, justifyContent: "center", flexWrap: "wrap" }}>
          <Link href={primary.href} className="nm-cta-primary" style={{ padding: "16px 30px", background: "linear-gradient(180deg,#9B5DE5 0%,#6D28D9 100%)", color: "#FFFFFF", fontFamily: sans, fontSize: 15, fontWeight: 600, textDecoration: "none", borderRadius: 9999, boxShadow: "0 10px 30px rgba(138,57,192,0.35)" }}>{primary.label}</Link>
          {secondary && (
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <div className="nm-overview-grid" style={{ display: "grid", gridTemplateColumns: "0.36fr 0.64fr", gap: 56 }}>
            <div style={{ ...rise(".8s", "entry 0% cover 24%") }}>
              <div style={{ fontFamily: mono, fontSize: 12, letterSpacing: "0.2em", color: "#8A39C0", marginBottom: 16 }}>OVERVIEW</div>
              <div style={{ fontFamily: serif, fontSize: "clamp(20px, 3.4vw, 27px)", fontWeight: 600, lineHeight: 1.2, color: "#0B0420", letterSpacing: "-0.01em" }}>{data.overview.heading}</div>
            </div>
            <div>
              ...
              <div style={{ ...rise(".8s", "entry 0% cover 36%"), borderLeft: "3px solid #8A39C0", padding: "6px 0 6px 24px", marginTop: 6 }}>
                <div style={{ fontFamily: serif, fontStyle: "italic", fontSize: "clamp(20px, 3.4vw, 25px)", fontWeight: 500, lineHeight: 1.32, color: "#0B0420" }}>{data.overview.pullquote}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: "#FFFFFF", padding: "clamp(56px, 11vw, 100px) clamp(20px, 5vw, 32px)", borderTop: "1px solid #E2E8F1" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          <div style={{ ...rise(".8s", "entry 0% cover 24%"), display: "flex", alignItems: "baseline", gap: 16, flexWrap: "wrap", margin: "0 0 44px" }}>
            <h2 style={{ fontFamily: mono, fontSize: 12, letterSpacing: "0.2em", color: "#8A39C0", margin: 0 }}>{data.sectionLabel}</h2>
          ...
          <div className="nm-feature-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 20 }}>
            {data.features.map((c) => (
              <div key={c.name} className="nm-feature-card" style={{ ...rise(".8s", "entry 0% cover 30%"), background: "#FFFFFF", border: "1px solid #E2E8F1", borderRadius: 18, padding: "clamp(20px, 4vw, 30px)" }}>
                <h3 style={{ fontFamily: sans, fontWeight: 700, fontSize: "clamp(15.5px, 2.4vw, 18px)", color: "#0B0420", margin: "0 0 10px", letterSpacing: "-0.01em" }}>{c.name}</h3>
          ...
          {data.stack && data.stack.length > 0 && (
            <div style={{ ...rise(".8s", "entry 0% cover 38%"), marginTop: 30, background: "#FFFFFF", border: "1px solid #E2E8F1", borderRadius: 14, padding: "clamp(18px, 3.5vw, 24px) clamp(20px, 4vw, 28px)" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {data.stack.map((t) => (
                  <span key={t} style={{ fontFamily: mono, fontSize: 11, color: "#475569", background: "#F5F2FC", borderRadius: 9999, padding: "5px 11px" }}>{t}</span>
                ))}
              </div>
            </div>
          )}
