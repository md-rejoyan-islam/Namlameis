...
const archLayers = [
  { title: "CLIENT (React / Next.js)", sub: "Portal · Intake · Dashboard · Trust · Hub", accent: "#8A39C0" },
  { title: "API LAYER (Node / Go · REST)", sub: "Auth · validation · rate-limit · webhooks", accent: "#6D28D9" },
  { title: "DATA (Encrypted store)", sub: "Postgres / Mongo · audit log · secrets vault", accent: "#8A39C0" },
];
...
export default function ProductsPage() {
  return (
    <div style={{ background: "#FFFFFF" }}>
      ...
      <section style={{ background: "#FFFFFF", padding: "clamp(48px, 11vw, 96px) clamp(20px, 5vw, 32px)" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          <div style={{ maxWidth: 620, margin: "0 0 48px" }}>
            <div style={{ ...rise(".8s", "entry 0% cover 24%"), fontFamily: mono, fontSize: 12, letterSpacing: "0.2em", color: "#8A39C0", marginBottom: 16 }}>THE PRODUCT SUITE</div>
          ...
          <div className="nm-suite-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 22 }}>
            {productSuite.map((p) => (
              <Link key={p.slug} href={`/products/${p.slug}`} className="nm-suite-card" style={{ ...rise(".8s", "entry 0% cover 30%"), textDecoration: "none", background: "#FFFFFF", border: "1px solid #E2E8F1", borderTop: `3px solid ${p.accent}`, borderRadius: 16, padding: "clamp(20px, 4vw, 30px) clamp(18px, 4vw, 28px)", display: "flex", flexDirection: "column" }}>
                ...
                <h3 style={{ fontFamily: sans, fontWeight: 600, fontSize: "clamp(15.5px, 2.4vw, 19px)", color: "#0B0420", margin: "0 0 9px", letterSpacing: "-0.01em" }}>{p.name}</h3>
      ...
      {/* ARCHITECTURE */}
      <section style={{ background: "#FFFFFF", padding: "clamp(50px, 11vw, 100px) clamp(20px, 5vw, 32px)", borderTop: "1px solid #E2E8F1" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <div style={{ maxWidth: 640, margin: "0 0 44px" }}>
            <div style={{ ...rise(".8s", "entry 0% cover 24%"), fontFamily: mono, fontSize: 12, letterSpacing: "0.2em", color: "#8A39C0", marginBottom: 16 }}>PLATFORM ARCHITECTURE</div>
          ...
          <figure style={{ ...rise("1s", "entry 0% cover 30%"), margin: 0 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 760 }}>
              {archLayers.map((l, i) => (
                <div key={l.title}>
                  <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F1", borderLeft: `5px solid ${l.accent}`, borderRadius: 12, padding: "clamp(14px, 4vw, 22px) clamp(17px, 4vw, 26px)" }}>
                    <div style={{ fontFamily: sans, fontWeight: 600, fontSize: "clamp(15.5px, 2.4vw, 17px)", color: "#0B0420", marginBottom: 4 }}>{l.title}</div>
              ...
              <div style={{ background: "#0B0420", borderRadius: 12, padding: "clamp(14px, 4vw, 22px) clamp(17px, 4vw, 26px)", marginTop: 6 }}>
                <div style={{ fontFamily: mono, fontSize: 11, letterSpacing: "0.16em", color: "#8A39C0", marginBottom: 8 }}>SECURITY BY DESIGN</div>
      ...
      <section style={{ background: "#120428", padding: "clamp(48px, 11vw, 96px) clamp(20px, 5vw, 32px)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          <div style={{ ...rise(".8s", "entry 0% cover 26%"), fontFamily: mono, fontSize: 11, letterSpacing: "0.2em", color: "#8A39C0", marginBottom: 24 }}>A CANDID DISTINCTION</div>
