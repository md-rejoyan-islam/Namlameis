      ...
      <section style={{ background: "#FFFFFF", padding: "clamp(50px, 11vw, 100px) clamp(20px, 5vw, 32px)" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <div className="nm-overview-grid" style={{ display: "grid", gridTemplateColumns: "0.52fr 0.48fr", gap: 56, alignItems: "center" }}>
            ...
            <figure style={{ ...rise("1s", "entry 0% cover 30%"), margin: 0, border: "1px solid #E2E8F1", borderRadius: 18, background: "#FFFFFF", padding: "clamp(20px, 4vw, 30px)" }}>
              ...
              <svg viewBox="0 0 320 290" style={{ width: "100%", display: "block" }}>
                ...
                <circle cx="160" cy="155" r="6" fill="none" stroke="#16B9A6" strokeWidth="1.2">
          ...
          <div style={{ ...rise(".8s", "entry 0% cover 34%"), borderLeft: "3px solid #16B9A6", padding: "8px 0 8px 28px", margin: "48px auto 0", maxWidth: 820 }}>
            <div style={{ fontFamily: serif, fontStyle: "italic", fontSize: "clamp(20px, 3.4vw, 27px)", lineHeight: 1.32, color: "#0A1424" }}>The institutions worth defending are the ones that remember what they are for. Our task is to ensure they remain.</div>
          </div>
        </div>
      </section>

      {/* GOALS */}
      <section style={{ background: "#0B1A2E", padding: "clamp(50px, 11vw, 100px) clamp(20px, 5vw, 32px)", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {goals.map((g) => (
              <div key={g.no} className="nm-goal-row" style={{ ...rise(".8s", "entry 0% cover 32%"), display: "grid", gridTemplateColumns: "64px 0.3fr 0.7fr", gap: 24, alignItems: "start", background: "rgba(247,249,252,0.03)", border: "1px solid rgba(226,232,241,0.1)", borderRadius: 15, padding: "clamp(20px, 4vw, 30px) clamp(21px, 4vw, 32px)" }}>
                <div style={{ fontFamily: serif, fontSize: "clamp(26px, 6vw, 40px)", color: "#16B9A6", lineHeight: 0.9 }}>{g.no}</div>
