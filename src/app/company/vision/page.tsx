    ...
    <div style={{ background: "#FFFFFF" }}>
      ...
      <section style={{ position: "relative", overflow: "hidden", background: "radial-gradient(120% 100% at 50% 0%,#1A0840 0%,#120428 52%,#080214 100%)" }}>
        ...
        <div style={{ position: "relative", maxWidth: 1080, margin: "0 auto", padding: "clamp(52px, 13vw, 86px) clamp(20px, 5vw, 32px) clamp(41px, 10vw, 74px)", textAlign: "center" }}>
          <div style={{ ...rise(".7s", "entry 0% cover 18%"), fontFamily: mono, fontSize: 11.5, letterSpacing: "0.24em", color: "#8A39C0", marginBottom: 26 }}>VISION · 2024 →</div>
          <h1 style={{ ...rise(".85s", "entry 0% cover 24%"), fontFamily: serif, fontWeight: 400, fontSize: "clamp(30px, 6.4vw, 52px)", lineHeight: 1.08, letterSpacing: "-0.02em", color: "#FFFFFF", margin: "0 auto", maxWidth: 880 }}>
            The next decade will be fought in <span style={{ fontStyle: "italic", color: "#8A39C0" }}>code</span>, <span style={{ fontStyle: "italic", color: "#6D28D9" }}>narrative</span>, and <span style={{ fontStyle: "italic", color: "#8A39C0" }}>trust</span>.
          </h1>
        </div>
      </section>
      ...
      <section style={{ background: "#FFFFFF", padding: "clamp(50px, 11vw, 100px) clamp(20px, 5vw, 32px)" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <div className="nm-overview-grid" style={{ display: "grid", gridTemplateColumns: "0.52fr 0.48fr", gap: 56, alignItems: "center" }}>
            <div>
              <div style={{ ...rise(".8s", "entry 0% cover 24%"), fontFamily: mono, fontSize: 12, letterSpacing: "0.2em", color: "#8A39C0", marginBottom: 18 }}>THE VISION</div>
            ...
            <figure style={{ ...rise("1s", "entry 0% cover 30%"), margin: 0, border: "1px solid #E2E8F1", borderRadius: 18, background: "#FFFFFF", padding: "clamp(20px, 4vw, 30px)" }}>
              ...
              <svg viewBox="0 0 320 290" style={{ width: "100%", display: "block" }}>
                <circle cx="160" cy="105" r="78" fill="rgba(138,57,192,0.12)" stroke="#8A39C0" strokeWidth="1.2" />
                <circle cx="118" cy="180" r="78" fill="rgba(109,40,217,0.12)" stroke="#6D28D9" strokeWidth="1.2" />
                <circle cx="202" cy="180" r="78" fill="rgba(138,57,192,0.12)" stroke="#8A39C0" strokeWidth="1.2" />
                <text x="160" y="70" textAnchor="middle" fill="#0B0420" fontFamily={mono} fontSize="10">CYBER</text>
                <text x="74" y="206" textAnchor="middle" fill="#0B0420" fontFamily={mono} fontSize="10">AI</text>
                <text x="246" y="206" textAnchor="middle" fill="#0B0420" fontFamily={mono} fontSize="10">NARRATIVE</text>
                <circle cx="160" cy="155" r="6" fill="none" stroke="#8A39C0" strokeWidth="1.2">
                ...
                <circle cx="160" cy="155" r="6" fill="#8A39C0" style={{ animation: "nm-pulse 2.6s ease-in-out infinite" }} />
              </svg>
            </figure>
          </div>
          <div style={{ ...rise(".8s", "entry 0% cover 34%"), borderLeft: "3px solid #8A39C0", padding: "8px 0 8px 28px", margin: "48px auto 0", maxWidth: 820 }}>
            <div style={{ fontFamily: serif, fontStyle: "italic", fontSize: "clamp(20px, 3.4vw, 27px)", lineHeight: 1.32, color: "#0B0420" }}>The institutions worth defending are the ones that remember what they are for. Our task is to ensure they remain.</div>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#120428", padding: "clamp(50px, 11vw, 100px) clamp(20px, 5vw, 32px)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 1080, margin: "0 auto" }}>
          <h2 style={{ ...rise(".8s", "entry 0% cover 24%"), fontFamily: mono, fontSize: 12, letterSpacing: "0.2em", color: "#8A39C0", margin: "0 0 44px" }}>OUR GOALS</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {goals.map((g) => (
              <div key={g.no} className="nm-goal-row" style={{ ...rise(".8s", "entry 0% cover 32%"), display: "grid", gridTemplateColumns: "64px 0.3fr 0.7fr", gap: 24, alignItems: "start", background: "rgba(247,249,252,0.03)", border: "1px solid rgba(226,232,241,0.1)", borderRadius: 15, padding: "clamp(20px, 4vw, 30px) clamp(21px, 4vw, 32px)" }}>
                <div style={{ fontFamily: serif, fontSize: "clamp(26px, 6vw, 40px)", color: "#8A39C0", lineHeight: 0.9 }}>{g.no}</div>
